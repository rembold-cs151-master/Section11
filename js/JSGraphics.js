 ...
 * package.  The typical usage pattern for this file is to include a script
 * tag that reads in this file in the head section of the index.html file,
 * as follows:
 *
...
const PGL_VERSION = 0.94;
const PGL_BUGFIX = 4;
const PGL_DATE = "8-Apr-21";
 ...
 * by id.  If id is missing, it defaults to "JSGraphics"; if width and
 * height are missing, the size of the size of the GWindow must be set
 * using CSS.
 */
 ...
 */
    ...
    function addEventListener(type, listener) {
        ...
        if (type == "press") type = "mousedown";
        if (type == "release") type = "mouseup";
...
/*
 * Method: setTimeout
 * Usage: gw.setTimeout(fn, delay);
 * --------------------------------
 * Calls fn after delay milliseconds.
 */
...
/* Protected methods */
        ...
        _conditionalRepaint: _conditionalRepaint,
        _paint: _paint,
        _paintComponent: _paintComponent,
        _sendBackward: _sendBackward,
        _sendForward: _sendForward,
        _sendToBack: _sendToBack,
        _sendToFront: _sendToFront,
        _updateMouseEvent: _updateMouseEvent
...
/* Protected methods */
    ...
    function _paint(ctx) {
        if (visible) {
            ctx.save();
            ctx.transform(ctm.a, ctm.b, ctm.c, ctm.d, x + ctm.tx, y + ctm.ty);
 ...
 * which is identified by a hyphen followed by a digit.
 */

    glabel.setFont = function(str) {
        font = str;
        let h1 = str.indexOf("-");
        if (h1 > 0 && h1 < str.length - 1) {
            let ch = str.charAt(h1 + 1);
            if (ch < "0" || ch > "9") h1 = -1;
        }
        if (h1 === -1) {
            fontTag = str;
 ...
 */
    ...
    gpoly.addPolarEdge = function(r, theta) {
        gpoly.addEdge(r * GMath.cosDegrees(theta),
...
/* Protected methods */
    ...
    gimage._paintTransformed = function(ctx) {
        if (this._image.complete) {
            ctx.drawImage(this._image, 0, 0);
        } else if (gimage.getParent()) {
            let observer = gimage.getWindow();
    ...
    function checkComplete() {
        if (image.complete) {
            ...
            setTimeout(checkComplete, TIMEOUT);
        }
    }
...
/*
 * Class method: createPixelArray
 * Usage: let array = GImage.createPixelArray(width, height);
 * ----------------------------------------------------------
 * Creates an array of integers with the specified dimensions.
 */

GImage.createPixelArray = function(width, height) {
    let line = [ ];
    ...
    let array = [ ];
    for (let i = 0; i < height; i++) {
        array[i] = (i === 0) ? line : line.slice(0);
    }
    return array;
};

/*
 * Class method: chooseImage
 * Usage: GImage.chooseImage(callback);
 *        GImage.chooseImage(callback, cancelCallback);
 * ----------------------------------------------------
 * Creates a file chooser and allows the user to select an image file.
 * When the resulting image is fully loaded, JavaScript calls the
 * specified callback function, passing in the GImage as a parameter.
 * Clicking Cancel in the file dialog ordinarily has no effect, but
 * the caller can trigger an action in this case by supplying a
 * function as the cancelCallback parameter.
 */

GImage.chooseImage = function(callback, cancelCallback) {
    ...
    function gotDataURL(data) {
        let image = GImage(data);
    ...
    function changeAction() {
        ...
        if (files.length > 0) {
            ...
            rd.onload = function() { gotDataURL(rd.result); };
 ...
 * Class: GState
 * -------------
 * This class defines an empty object to which the client can add fields.
 * It exists for compatibility with those implementations that do not
 * support closures.
 */
...
/* Extended classes for the PowerPoint functions */

/*
 * Class: GOvalCallout
 * -------------------
 * This class is a graphical object whose appearance consists of an oval
 * with a callout wedge.
 */

/*
 * Factory method: GOvalCallout
 * Usage: let callout = GOvalCallout(x, y, width, height);
 * -------------------------------------------------------
 * Creates a new GOvalCallout object that fits inside the specified bounds.
 * Clients must call setWedgePoint to create the wedge point.
 */

function GOvalCallout(x, y, width, height) {

    const WEDGE_DTHETA = 11 * Math.PI / 180;
    let callout = GObject();
    callout.setLocation(x, y);
    callout.wedgeX = undefined;
    callout.wedgeY = undefined;

    callout.toString = function() {
        let x = callout.getX();
        let y = callout.getY();
        return "GOvalCallout(" + x + ", " + y + ", " +
                             width + ", " + height + ")";
    };

    callout.setWedgePoint = function(x, y) {
        callout.wedgeX = x;
        callout.wedgeY = y;
    }

    callout._getBounds = function(ctm) {
        let bb = GRectangle(ctm.transform(0, 0));
        bb.add(ctm.transform(width, 0));
        bb.add(ctm.transform(0, height));
        bb.add(ctm.transform(width, height));
    ...
    callout._contains = function(x, y) {
        let rx = width / 2;
        let ry = height / 2;
        let tx = x - rx;
        let ty = y - ry;
    ...
    callout._paintTransformed = function(ctx) {
        let x = callout.getX();
        let y = callout.getY();
        ctx.translate(width / 2, height / 2);
        ctx.scale(1, height / width);
        ctx.beginPath();
        if (callout.wedgeX === undefined) {
            ctx.arc(0, 0, width / 2, 0, 2 * Math.PI, true);
        } else {
            let theta = Math.atan2(callout.wedgeY, callout.wedgeX);
            let startAngle = theta - WEDGE_DTHETA;
            let endAngle = theta + WEDGE_DTHETA;
            ctx.arc(0, 0, width / 2, startAngle, endAngle, true);
            ctx.lineTo(callout.wedgeX, callout.wedgeY * width / height);
        }
        ctx.closePath();
        ctx.scale(1, width / height);
        if (callout.isFilled()) {
            let fillColor = callout.getFillColor();
            if (!fillColor) fillColor = callout.getColor();
            ctx.fillStyle = fillColor;
            ctx.fill();
        }
        ctx.strokeStyle = callout.getColor();
        ctx.stroke();
    };
